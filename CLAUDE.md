...
| 模态 | 状态 | 信号质量 | 融合优先级 |
|------|------|---------|-----------|
| **视频 PPG** | ✅ 当前重点 | 红通道 8/10 HR检测 | 最高（先验证） |
| **音频 PCG** | ✅ 全部98视频含AAC音轨 | 部分可听到心跳声 | 第二（视频后） |
| **IMU BCG** | ✅ acc_z含心跳微震动 | 75%样本可检测HR (r=0.31-0.63) | 第三 |

> 💡 多模态融合路线：视频单模态验证 → +IMU (cross-attention, 已实现) → +音频 (Mel spectrogram + cross-attention)
...
- 音频模态：需评估心音清晰度分布，实现 PCG 特征提取 + 融合
